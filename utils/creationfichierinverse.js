...
async function buildInvertedIndexWithTFIDF() {
    try {
        const documents = await Document.find(); // getting all documents
        ...
        for (const doc of documents) {
            ...
            for (const term of index) {
                ...
                if (frequency[term] === undefined || isNaN(frequency[term])) {
                    continue;
                }
                
                // Use `frequency[term]` instead of `frequency.get(term)`
                const tf = frequency[term]; // Frequency of the term in the document
